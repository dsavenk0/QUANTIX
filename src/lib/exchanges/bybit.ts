'use client';

import { ExchangeClient } from './client';
import { OHLCV, OrderBookData, WebSocketMessageHandler } from './common';

const API_BASE_URL = 'https://api.bybit.com/v5/market';
const WS_BASE_URL = 'wss://stream.bybit.com/v5/public/spot';

class BybitClient implements ExchangeClient {
    readonly name = 'bybit';

    private intervalMap: { [key: string]: string } = {
        '1m': '1',
        '5m': '5',
        '30m': '30',
        '1h': '60',
        '4h': '240',
        '1d': 'D',
    };

    formatApiSymbol(pair: string): string {
        return pair.replace('-', '').toUpperCase();
    }

    formatPair(apiSymbol: string): string {
        const quoteAssets = ['USDT', 'BUSD', 'USDC', 'TUSD'];
        const quoteAsset = quoteAssets.find(qa => apiSymbol.endsWith(qa));
        
        if (quoteAsset) {
            const baseAsset = apiSymbol.substring(0, apiSymbol.length - quoteAsset.length);
            return `${baseAsset.toLowerCase()}-${quoteAsset.toLowerCase()}`;
        }
        return apiSymbol.toLowerCase();
    }

    async fetchKlines(symbol: string, interval: string): Promise<OHLCV[]> {
        const apiSymbol = this.formatApiSymbol(symbol);
        const bybitInterval = this.intervalMap[interval];
        if (!bybitInterval) throw new Error(`Interval not supported by Bybit: ${interval}`);
        
        const response = await fetch(`${API_BASE_URL}/kline?category=spot&symbol=${apiSymbol}&interval=${bybitInterval}&limit=300`);
        const data = await response.json();

        if (data.retCode !== 0) {
            throw new Error(`Bybit API error: ${data.retMsg}`);
        }

        return data.result.list.map((d: any[]) => ({
            time: parseInt(d[0]) / 1000,
            open: parseFloat(d[1]),
            high: parseFloat(d[2]),
            low: parseFloat(d[3]),
            close: parseFloat(d[4]),
            value: parseFloat(d[6]), // Quote asset volume
            delta: 0,
        })).reverse(); // Bybit returns in descending order
    }

    async fetchOrderBook(symbol: string): Promise<OrderBookData> {
        const apiSymbol = this.formatApiSymbol(symbol);
        const response = await fetch(`${API_BASE_URL}/orderbook?category=spot&symbol=${apiSymbol}&limit=50`);
        const data = await response.json();
        
        if (data.retCode !== 0) {
            throw new Error(`Bybit API error: ${data.retMsg}`);
        }

        return {
            bids: data.result.b.map(([price, quantity]: [string, string]) => [parseFloat(price), parseFloat(quantity)]),
            asks: data.result.a.map(([price, quantity]: [string, string]) => [parseFloat(price), parseFloat(quantity)]),
        };
    }

    async fetchAllSymbols(): Promise<string[]> {
        const response = await fetch(`${API_BASE_URL}/instruments-info?category=spot&limit=1000`);
        const data = await response.json();

        if (data.retCode !== 0) {
            throw new Error(`Bybit API error: ${data.retMsg}`);
        }

        return data.result.list
            .filter((s: any) => 
                s.status === 'Trading' && 
                (s.quoteCoin === 'USDT' || s.quoteCoin === 'USDC')
            )
            .map((s: any) => this.formatPair(s.symbol))
            .sort();
    }

    connect(symbol: string, interval: string, handler: WebSocketMessageHandler): () => void {
        let ws: WebSocket | null = null;
        let pingInterval: NodeJS.Timeout | null = null;
        let reconnectTimer: NodeJS.Timeout | null = null;
        let shouldReconnect = true;

        const bybitInterval = this.intervalMap[interval];
        const apiSymbol = this.formatApiSymbol(symbol);
        
        const book: OrderBookData = { bids: [], asks: [] };

        const applyBookUpdate = (side: 'bids' | 'asks', updates: [string, string][]) => {
            updates.forEach(([price, size]) => {
                const priceNum = parseFloat(price);
                const sizeNum = parseFloat(size);
                const bookSide = book[side];
                const index = bookSide.findIndex(level => level[0] === priceNum);

                if (sizeNum === 0) {
                    if (index !== -1) bookSide.splice(index, 1);
                } else {
                    if (index !== -1) {
                        bookSide[index][1] = sizeNum;
                    } else {
                        bookSide.push([priceNum, sizeNum]);
                    }
                }
            });

            if (side === 'bids') {
                book.bids.sort((a, b) => b[0] - a[0]);
            } else {
                book.asks.sort((a, b) => a[0] - b[0]);
            }
        };

        const handleReconnect = () => {
            if (!shouldReconnect) return;
            if (reconnectTimer) clearTimeout(reconnectTimer);
            reconnectTimer = setTimeout(performConnect, 5000);
        };
        
        const performConnect = () => {
            if (!shouldReconnect) return;

            ws = new WebSocket(WS_BASE_URL);

            ws.onopen = () => {
                ws?.send(JSON.stringify({
                    op: 'subscribe',
                    args: [
                        `orderbook.50.${apiSymbol}`,
                        `kline.${bybitInterval}.${apiSymbol}`,
                        `publicTrade.${apiSymbol}`
                    ]
                }));

                if (pingInterval) clearInterval(pingInterval);
                pingInterval = setInterval(() => {
                    if (ws && ws.readyState === WebSocket.OPEN) {
                        ws.send(JSON.stringify({ op: 'ping' }));
                    }
                }, 20000);
            };

            ws.onmessage = (event) => {
                const message = JSON.parse(event.data);

                if (message.topic?.startsWith('orderbook')) {
                    const data = message.data;
                    if (!data) return;

                    if (message.type === 'snapshot') {
                        book.bids = (data.b || []).map(([p, q]: [string, string]) => [parseFloat(p), parseFloat(q)]);
                        book.asks = (data.a || []).map(([p, q]: [string, string]) => [parseFloat(p), parseFloat(q)]);
                    } else if (message.type === 'delta') {
                        if (data.b) applyBookUpdate('bids', data.b);
                        if (data.a) applyBookUpdate('asks', data.a);
                    }
                    
                    handler({ 
                        stream: message.topic, 
                        type: 'depth', 
                        data: { 
                            bids: [...book.bids].slice(0, 50),
                            asks: [...book.asks].slice(0, 50)
                        }
                    }, this.name);
                } else if (message.topic?.startsWith('kline')) {
                     const candle = message.data[0];
                     const newCandle: Partial<OHLCV> = {
                        time: candle.start / 1000,
                        open: parseFloat(candle.open),
                        high: parseFloat(candle.high),
                        low: parseFloat(candle.low),
                        close: parseFloat(candle.close),
                        value: parseFloat(candle.turnover),
                    };
                    handler({ stream: message.topic, type: 'kline', data: newCandle }, this.name);
                } else if (message.topic?.startsWith('publicTrade')) {
                    const trades = message.data;
                    trades.forEach((trade: any) => {
                        handler({
                            stream: message.topic,
                            type: 'trade',
                            data: {
                                price: parseFloat(trade.p),
                                quantity: parseFloat(trade.v),
                                time: parseInt(trade.T),
                                isBuyerMaker: trade.S === 'Sell', // S is Taker Side. 'Buy' means taker is buyer. isBuyerMaker is true when Taker is Seller.
                            }
                        }, this.name);
                    });
                }
            };

            ws.onerror = (error) => {
                // WebSocket Error
            };

            ws.onclose = () => {
                ws = null;
                if (pingInterval) clearInterval(pingInterval);

                if (!shouldReconnect) {
                    return;
                }
                handleReconnect();
            };
        };

        performConnect();

        return () => {
            shouldReconnect = false;
            if (reconnectTimer) {
                clearTimeout(reconnectTimer);
                reconnectTimer = null;
            }
            if (pingInterval) {
                clearInterval(pingInterval);
                pingInterval = null;
            }
            
            if (ws) {
                if (ws.readyState === WebSocket.OPEN) {
                     ws.send(JSON.stringify({
                        op: 'unsubscribe',
                        args: [
                            `orderbook.50.${apiSymbol}`,
                            `kline.${bybitInterval}.${apiSymbol}`,
                            `publicTrade.${apiSymbol}`
                        ]
                    }));
                }
                ws.onopen = null;
                ws.onmessage = null;
                ws.onerror = null;
                ws.onclose = null;
                ws.close();
                ws = null;
            }
        };
    }
}

export const bybitClient = new BybitClient();
